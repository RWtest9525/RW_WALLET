...
const hydrateInstantShell = () => {
    ...
    if (lastUser) {
        ...
        try {
            ...
            if (effectiveUser === ADMIN_UID || isImpersonating ||
                cachedRole === 'admin' || cachedRole === 'subadmin' || cachedRole === 'owner' ||
                (cachedUser && (cachedUser.role === 'admin' || cachedUser.role === 'subadmin' || cachedUser.role === 'owner' || cachedUser.isAdmin))) {
                isUserAdmin = true;
            }
        } catch (_) {}
