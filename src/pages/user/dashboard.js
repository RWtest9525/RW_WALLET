...
const UNITY_ADS_CONFIG = {
    gameId: '800108003',
    projectId: '4817a253-8be7-49e5-9a8c-a6cc87d9ba76',
    placementId: 'Rewarded_Android'
};

const WATCH_AD_CARDS = [
    {
        id: 'ad_full_60',
        title: 'Full 60s Unity Video Ad',
        description: 'Watch complete 60-second video ad & earn 50% ad revenue share',
        reward: 0.50,
        duration: 60,
        badge: '🔥 60s FULL AD',
        badgeColor: 'amber',
    ...
    {
        id: 'ad_speed_30',
        title: 'Speed Video Ad',
        description: 'Watch a 30-second Unity video ad & earn instant cash',
        reward: 0.25,
        duration: 30,
        badge: '⚡ 30s SPEED AD',
        badgeColor: 'emerald',
    ...
    },
    {
        ...
        description: 'Watch a quick 15-second Unity video ad & earn cash',
        reward: 0.15,
        duration: 15,
        badge: '✨ 15s QUICK AD',
        badgeColor: 'blue',
    ...
    {
        id: 'ad_mega_60',
        title: '60s Mega Unity Mission',
        description: 'Watch complete 60s high-paying video ad mission',
        reward: 0.60,
        duration: 60,
        badge: '💎 60s MEGA REWARD',
        badgeColor: 'purple',
    ...
    {
        id: 'ad_bonus_60',
        title: '60s Daily Bonus Ad',
        description: 'Claim your daily exclusive 60-second video ad bonus',
        reward: 0.50,
        duration: 60,
        badge: '🎁 60s DAILY BONUS',
        badgeColor: 'rose',
...
const getTodayAdStatsKey = (uid) => {
    const today = new Date().toISOString().split('T')[0];
    return `rw_watch_ads_stats_${uid}_${today}`;
};

const getWatchAdDailyStats = (uid) => {
    try {
        const raw = localStorage.getItem(getTodayAdStatsKey(uid));
...
const saveWatchAdDailyStats = (uid, stats) => {
    try {
        localStorage.setItem(getTodayAdStatsKey(uid), JSON.stringify(stats));
    } catch (e) {}
};

const showWatchAdsPage = async () => {
    const uid = currentUser?.uid || 'guest';
    const userDocData = window.currentUserData || {};
    const balance = Number(userDocData.balance || 0).toFixed(2);
    const totalAdEarnings = Number(userDocData.totalAdEarnings || 0).toFixed(2);
    const dailyStats = getWatchAdDailyStats(uid);

    let totalWatchedToday = 0;
    WATCH_AD_CARDS.forEach(c => {
        totalWatchedToday += (dailyStats[c.id] || 0);
    });

    const badgeColorClasses = {
        emerald: 'bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300 border-emerald-300 dark:border-emerald-800',
        amber: 'bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300 border-amber-300 dark:border-amber-800',
        purple: 'bg-purple-100 text-purple-800 dark:bg-purple-950/60 dark:text-purple-300 border-purple-300 dark:border-purple-800',
        blue: 'bg-blue-100 text-blue-800 dark:bg-blue-950/60 dark:text-blue-300 border-blue-300 dark:border-blue-800',
        rose: 'bg-rose-100 text-rose-800 dark:bg-rose-950/60 dark:text-rose-300 border-rose-300 dark:border-rose-800'
    };

    const cardsHtml = WATCH_AD_CARDS.map(card => {
        const completedToday = dailyStats[card.id] || 0;
        const remaining = Math.max(0, card.dailyLimit - completedToday);
        const isLimitReached = remaining <= 0;

        return `
            <div class="relative overflow-hidden rounded-3xl border border-slate-200 bg-white p-5 shadow-lg transition-all hover:shadow-xl dark:border-slate-700 dark:bg-gray-800 flex flex-col justify-between space-y-4">
                <div class="flex items-start justify-between gap-3">
                    <div class="flex items-center gap-3">
                        <div class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-slate-100 to-slate-200 dark:from-gray-700 dark:to-gray-900 p-2.5 shadow-inner">
                            <img src="${card.icon}" alt="${card.title}" class="h-9 w-9 object-contain" loading="eager" decoding="async">
                        </div>
                        <div>
                            <span class="inline-block rounded-full border px-2.5 py-0.5 text-[10px] font-black uppercase tracking-wider ${badgeColorClasses[card.badgeColor] || badgeColorClasses.amber}">
                                ${card.badge}
                            </span>
                            <h4 class="mt-1 text-base font-black text-slate-900 dark:text-white leading-tight">${card.title}</h4>
                            <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">${card.description}</p>
                        </div>
                    </div>
                </div>

                <div class="flex items-center justify-between border-t border-slate-100 pt-3 dark:border-slate-700/60">
                    <div class="flex items-center gap-2">
                        <span class="inline-flex items-center gap-1 rounded-xl bg-emerald-50 px-3 py-1.5 text-xs font-black text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800">
                            💰 +₹${card.reward.toFixed(2)}
                        </span>
                        <span class="inline-flex items-center gap-1 rounded-xl bg-slate-100 px-2.5 py-1.5 text-xs font-bold text-slate-600 dark:bg-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-600">
                            ⏱️ ${card.duration}s
                        </span>
                    </div>
                    <span class="text-[11px] font-bold text-slate-500 dark:text-slate-400">
                        ${completedToday}/${card.dailyLimit} Completed Today
                    </span>
                </div>

                <button type="button" 
                    data-action="start-watch-ad"
                    data-adid="${card.id}"
                    class="btn-start-watch-ad w-full py-3.5 px-4 rounded-2xl font-black text-sm text-center transition-all shadow-md active:scale-95 ${
                        isLimitReached 
                    ...
                    ${isLimitReached ? '✓ Daily Limit Reached' : '▶ Watch Unity Video & Earn'}
                </button>
            </div>
        `;
    }).join('');
    ...
    const content = `
        ${getPageHeader('Watch Ads & Earn')}
        <div class="mx-auto max-w-xl px-4 pb-28 space-y-5">
            <!-- Header Banner -->
            <div class="relative overflow-hidden rounded-3xl bg-gradient-to-br from-indigo-900 via-blue-900 to-slate-900 p-6 text-white shadow-2xl">
                <div class="absolute -right-10 -top-10 h-40 w-40 rounded-full bg-blue-500/20 blur-2xl"></div>
                <div class="absolute -left-10 -bottom-10 h-40 w-40 rounded-full bg-purple-500/20 blur-2xl"></div>
                
                <div class="relative z-10 flex items-center justify-between">
                    <div>
                        <div class="flex items-center gap-2">
                            <span class="h-2.5 w-2.5 rounded-full bg-emerald-400 animate-pulse"></span>
                            <span class="text-xs font-black uppercase tracking-wider text-blue-200">Unity Rewarded Ads</span>
                        </div>
                        <h3 class="mt-1 text-2xl font-black text-white">Watch Video Ads & Earn</h3>
                        <p class="mt-1 text-xs text-blue-100/80">Watch Unity video ads completely to receive instant cash into your wallet balance.</p>
                    </div>
                    <div class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-white/10 backdrop-blur border border-white/20 shadow-inner">
                        <img src="https://cdn-icons-png.flaticon.com/512/2659/2659360.png" alt="Ads" class="h-9 w-9 object-contain">
                    </div>
                </div>

                <div class="relative z-10 mt-6 grid grid-cols-3 gap-3 border-t border-white/10 pt-4">
                    <div class="rounded-2xl bg-white/10 p-3 backdrop-blur text-center">
                        <p class="text-[10px] font-bold uppercase text-blue-200">Wallet Balance</p>
                        <p class="text-base font-black text-emerald-400">₹${balance}</p>
                    </div>
                    <div class="rounded-2xl bg-white/10 p-3 backdrop-blur text-center">
                        <p class="text-[10px] font-bold uppercase text-blue-200">Total Ad Income</p>
                        <p class="text-base font-black text-amber-300">₹${totalAdEarnings}</p>
                    </div>
                    <div class="rounded-2xl bg-white/10 p-3 backdrop-blur text-center">
                        <p class="text-[10px] font-bold uppercase text-blue-200">Watched Today</p>
                        <p class="text-base font-black text-white">${totalWatchedToday}</p>
                    </div>
                </div>
            </div>

            <!-- Ad Policy Notice -->
            <div class="flex items-center gap-3 rounded-2xl bg-blue-50/80 p-3.5 text-xs text-blue-900 border border-blue-200 dark:bg-blue-950/40 dark:text-blue-200 dark:border-blue-800/60">
                <span class="text-lg">🛡️</span>
                <p class="leading-tight font-medium">Video ads only open when you explicitly click <b>Watch Unity Video & Earn</b>. No popups or video ads will ever show on other pages.</p>
            </div>

            <!-- Cards Grid -->
            <div class="space-y-4">
                <h4 class="text-sm font-black uppercase tracking-wider text-slate-800 dark:text-slate-200 flex items-center justify-between">
                    <span>Available Unity Ad Cards</span>
                    <span class="rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-bold text-blue-700 dark:bg-blue-900 dark:text-blue-300">${WATCH_AD_CARDS.length} Cards</span>
                </h4>
                ${cardsHtml}
            </div>
        </div>
    ...
    setTimeout(() => {
        document.querySelectorAll('.btn-start-watch-ad').forEach(btn => {
            btn.addEventListener('click', (e) => {
                const adId = btn.dataset.adid;
                const card = WATCH_AD_CARDS.find(c => c.id === adId);
                if (card) {
                    watchAdVideoModal(card);
                }
            });
        });
    }, 50);
...
const watchAdVideoModal = (adCard) => {
    const uid = currentUser?.uid;
    if (!uid) {
        return showNotification('Please sign in to watch ads and earn money.');
    }

    const dailyStats = getWatchAdDailyStats(uid);
    const completedToday = dailyStats[adCard.id] || 0;
    if (completedToday >= adCard.dailyLimit) {
    ...
    modalEl.innerHTML = `
        <div class="relative w-full max-w-lg overflow-hidden rounded-3xl bg-slate-900 border border-slate-800 text-white shadow-2xl flex flex-col">
            <!-- Modal Header -->
            <div class="flex items-center justify-between border-b border-slate-800 px-5 py-4 bg-slate-900/90">
                <div class="flex items-center gap-3">
                    <div class="flex h-10 w-10 items-center justify-center rounded-xl bg-blue-600/20 text-blue-400 border border-blue-500/30">
                        <img src="${adCard.icon}" class="h-6 w-6 object-contain" alt="Ad">
                    </div>
                    <div>
                ...
                <div class="flex items-center gap-2">
                    <span id="ad-timer-badge" class="rounded-full bg-amber-500/20 px-3 py-1 text-xs font-black text-amber-300 border border-amber-500/30 flex items-center gap-1.5">
            ...
            <!-- Progress Bar -->
            <div class="w-full bg-slate-800 h-2 overflow-hidden">
                <div id="ad-progress-bar" class="h-full bg-gradient-to-r from-blue-500 via-indigo-500 to-emerald-400 transition-all duration-1000" style="width: 0%;"></div>
            </div>
    ...
    timerInterval = setInterval(() => {
        ...
        if (secondsLeft <= 0) {
            ...
            if (timerBadgeEl) {
                timerBadgeEl.className = 'rounded-full bg-emerald-500/20 px-3 py-1 text-xs font-black text-emerald-300 border border-emerald-500/30';
            ...
            if (claimBtn) {
                ...
                claimBtn.className = 'w-full py-3.5 px-4 rounded-2xl font-black text-sm text-center transition-all bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white shadow-lg shadow-emerald-500/30 cursor-pointer active:scale-95 animate-bounce';
                claimBtn.innerHTML = '🎁 Claim ₹' + adCard.reward.toFixed(2) + ' Cash Reward';
            }
        }
    }, 1000);
    ...
    if (claimBtn) {
        claimBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (db && window.doc && window.updateDoc && window.increment) {
                    const userRef = window.doc(db, `artifacts/${appId}/public/data/users`, uid);
                    await window.updateDoc(userRef, {
                        balance: window.increment(adCard.reward),
                        totalAdEarnings: window.increment(adCard.reward),
                        updatedAt: window.serverTimestamp ? window.serverTimestamp() : Date.now()
                    });

                    if (window.collection && window.addDoc) {
                        const txRef = window.collection(db, `artifacts/${appId}/public/data/users/${uid}/transactions`);
                ...
                const dailyStats = getWatchAdDailyStats(uid);
                dailyStats[adCard.id] = (dailyStats[adCard.id] || 0) + 1;
                saveWatchAdDailyStats(uid, dailyStats);

                if (window.currentUserData) {
                    window.currentUserData.balance = Number((Number(window.currentUserData.balance || 0) + adCard.reward).toFixed(2));
                    window.currentUserData.totalAdEarnings = Number((Number(window.currentUserData.totalAdEarnings || 0) + adCard.reward).toFixed(2));
                }

                if (typeof window.showNotification === 'function') {
                    window.showNotification(`🎉 ₹${adCard.reward.toFixed(2)} credited to your wallet!`);
                }
                ...
                console.error('Failed to credit watch ad reward:', err);
                claimBtn.disabled = false;
                claimBtn.innerHTML = '🎁 Claim ₹' + adCard.reward.toFixed(2) + ' Cash Reward';
                if (typeof window.showNotification === 'function') {
                    window.showNotification('Failed to credit reward. Please check connection.');
                }
            }
        });
    }
...
const showTaskFeatureComingSoonPage = (feature = 'ads') => {
    if (feature === 'ads') {
        return showWatchAdsPage();
    }
    const isAds = false;
    const title = 'Daily Bonus';
    const icon = 'https://cdn-icons-png.flaticon.com/512/2611/2611152.png';
    const headline = 'Claim daily bonus, spin wheel and earn money upto 5 rupees.';
    const content = `
        ${getPageHeader(title)}
...
window.showWatchAdsPage = showWatchAdsPage;
window.watchAdVideoModal = watchAdVideoModal;
window.WATCH_AD_CARDS = WATCH_AD_CARDS;
window.UNITY_ADS_CONFIG = UNITY_ADS_CONFIG;
