                        ...
                        <div class="mt-3 pt-2 border-t border-gray-50 dark:border-gray-700/50 flex items-center justify-between gap-2">
                            ${canAdminManageTask(task) ? `
                                ...
                                <div class="flex gap-1">
                                    ${getAdminTaskIconButtonMini('open-task-submissions', task.id, 'Submissions', '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5.586a1 1 0 0 1 .707.293l5.414 5.414a1 1 0 0 1 .293.707V19a2 2 0 0 1-2 2z"></path>', 'amber')}
...
const showAdminTaskSubmissionsPage = async () => {
    const isUserAdmin = window.checkIsUserAdmin ? window.checkIsUserAdmin() : (currentUser?.uid === ADMIN_UID || currentUserData?.role === 'admin' || currentUserData?.role === 'subadmin');
    if (!currentUser || !isUserAdmin) return showNotification('Admin access only.', true);
    currentMainSection = 'admin';
