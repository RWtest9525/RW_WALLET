// scripts/transfer-all-owner-users.cjs
const APP_ID = 'digital-wallet-prod';
const OWNER_EMAIL = 'reviewsworld01@gmail.com';
const SUB_ADMIN_EMAIL = 'malasingh40163@gmail.com';

async function transferAllOwnerUsers() {
    console.log('Starting script to transfer all owner users to sub-admin...');
    ...
    for (const doc of docs) {
        ...
        if (currentParent === ownerUid || currentParent === 'ns1mO2Klg8OxFxe1pDirSXqrE923' || !fields.parentAdmin) {
            const userName = fields.name?.stringValue || fields.email?.stringValue || 'User';
            console.log(`Transferring user ${userId} (${userName}) to Sub-Admin ${subAdminUid}...`);
    ...
    console.log(`Transfer Complete! Total ${transferredCount} users transferred to Sub-Admin ${SUB_ADMIN_EMAIL} (${subAdminUid}).`);
}

transferAllOwnerUsers().catch(err => {
    console.error('Transfer script failed:', err);
});
