...
class InstagramExtractor(BaseExtractor):
    """
    Platform Extractor for Instagram Screenshots (Follow, Like, Comment tasks).
    Extensible module for future Instagram task verification.
    """

    def __init__(self):
        super().__init__(name="InstagramExtractor")

    def extract_review_card(self, img: np.ndarray) -> np.ndarray:
        if img is None or img.size == 0:
            return img
        h, w, _ = img.shape
        # Crop status bar and bottom navigation bar
