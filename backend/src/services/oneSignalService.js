...
function sanitizePayload(rawPayload) {
  ...
  for (const [k, v] of Object.entries(rawPayload)) {
    ...
    if (k === 'include_player_ids') {
      console.warn(`[OneSignal Sanitizer] ⚠️ Converting deprecated "include_player_ids" to standard "include_subscription_ids".`);
      out.include_subscription_ids = v;
      continue;
    }
    out[k] = v;
  }
...
async function sendPushToUser({ subscriptionIds, userIds, title, message, data, url } = {}) {
  ...
  if (cleanUserIds.length > 0) {
    ...
    console.log('[OneSignal sendPushToUser] → calling via include_aliases (external_id), with target_channel="push"');
    const r1 = await postNotificationApi(payloadAlias);
    results.push({ via: 'aliases.external_id', ...r1 });
  }
  ...
  if (cleanSubscriptionIds.length > 0) {
    ...
    console.log(`[OneSignal sendPushToUser] → calling via include_subscription_ids (${cleanSubscriptionIds.length} ids) — NO target_channel, per OneSignal rules`);
